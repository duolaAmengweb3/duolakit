---
name: prd-activate
description: Activate the Pro tier for prd-splitter using your Gumroad license key. Stored locally at ~/.duolakit/licenses.json. No telemetry.
...
1. **Parse the key.**
   ...
   - If no key was passed, print:
     ```
     usage: /prd-activate <license-key>

     The key arrives in your Gumroad receipt email after purchase.
     Buy: https://duolakit.gumroad.com/l/prd-splitter
     ```
     and stop.
...
3. **Interpret the exit code.**
   - `0` → success. The key was accepted and `~/.duolakit/licenses.json` now has a slot for `prd-splitter` with `valid: true`. Print:
     ```
     ✓ Pro activated for prd-splitter.
     ...
     The activation is local to this machine. Re-run /prd-activate <key>
...
This removes only the `prd-splitter` slot from `~/.duolakit/licenses.json`. Other duolakit plugin licenses are untouched.
