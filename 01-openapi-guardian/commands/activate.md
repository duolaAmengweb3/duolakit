...
description: Activate the Pro tier for openapi-guardian using the email you gave when you DM'd to buy Pro. Verified in real-time against the duolakit-license Worker. Stored locally at ~/.duolakit/licenses.json. No telemetry.
...
1. **Parse the email.** First argument. If missing, print usage:
   ```
   usage: /openapi-activate <your-purchase-email>
...
Removes only the `openapi-guardian` slot from `~/.duolakit/licenses.json`. Other duolakit plugin licenses are untouched. Refunds are handled manually: when the operator agrees to refund, they run `bash bin/admin.sh revoke <plugin> <email>` which removes the email from KV. Subsequent `check` calls fail within seconds.
