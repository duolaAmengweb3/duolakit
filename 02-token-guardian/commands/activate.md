---
name: token-activate
description: Activate the Pro tier for token-guardian using your Gumroad license key. Stored locally at ~/.duolakit/licenses.json. No telemetry.
...
1. **Parse the key.**
   ...
   - If no key was passed, print:
     ```
     usage: /token-activate <license-key>

     The key arrives in your Gumroad receipt email after purchase.
     Buy: https://duolakit.gumroad.com/l/token-guardian
     ```
     and stop.
...
3. **Interpret the exit code.**
   - `0` → success. The key was accepted and `~/.duolakit/licenses.json` now has a slot for `token-guardian` with `valid: true`. Print:
     ```
     ✓ Pro activated for token-guardian.

     You now have:
       - /token-route multi-provider routing recommendations
       - /token-calibrate persistent /cost calibration
       - 48h email support: noreply@duolakit.pages.dev

     The activation is local to this machine. Re-run /token-activate <key>
...
This removes only the `token-guardian` slot from `~/.duolakit/licenses.json`. Other duolakit plugin licenses are untouched.
